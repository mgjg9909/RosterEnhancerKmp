...
class SalaryCalculator(
    ...
    fun calculateDailySummary(shift: Shift): DailySummary {
        ...
        val startTime = shift.startTime?.let { java.time.LocalDateTime.of(shift.date.toJava(), it.toJava()) }
            ?: java.time.LocalDateTime.now()
        val endTime = shift.endTime?.let {
            var dt = java.time.LocalDateTime.of(shift.date.toJava(), it.toJava())
            if (dt.isBefore(startTime)) dt = dt.plusDays(1)
            dt
    ...
    private fun calculateOverlapHours(
        ...
        while (!dayStart.isAfter(dayEnd)) {
            val dayBase = dayStart.atStartOfDay()
            if (hourEnd < 24) {
                overlapMinutes += overlapMinutes(start, end, dayBase, dayBase.plusHours(hourEnd.toLong()))
            }
            if (hourStart > 0) {
                overlapMinutes += overlapMinutes(start, end, dayBase.plusHours(hourStart.toLong()), dayBase.plusHours(24))
            }
    ...
    private fun overlapMinutes(
        ...
        val from = if (aStart.isAfter(bStart)) aStart else bStart
        val to   = if (aEnd.isBefore(bEnd)) aEnd else bEnd
        return if (from.isBefore(to)) Duration.between(from, to).toMinutes() else 0L
    }
}
