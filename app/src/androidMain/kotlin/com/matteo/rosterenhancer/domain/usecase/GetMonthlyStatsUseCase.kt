...
class GetMonthlyStatsUseCase @Inject constructor(
    ...
    fun execute(
        ...
        while (weekStart.monthNumber == firstOfMonth.monthNumber) {
            val weekEnd = minOf(weekStart.plusDays(6), firstOfMonth.toJava().withDayOfMonth(firstOfMonth.toJava().lengthOfMonth()).toKotlin())
    ...
    private fun calculateTopCompanions(myWorkShifts: List<Shift>, allWorkShifts: List<Shift>, selfMatricola: String): List<TopCompanion> {
        ...
        for (shift in allWorkShifts) {
            ...
            if (overlapStart < overlapEnd) {
                val mins = java.time.Duration.between(overlapStart.toJava(), overlapEnd.toJava()).toMinutes()
