...
class FindRestSwapCandidatesUseCase @Inject constructor(
    ...
    private fun validateTimeline(timeline: List<Shift>, userNameDesc: String): String? {
        ...
        for (i in 0 until workShifts.size - 1) {
            ...
            val daysBetween = java.time.temporal.ChronoUnit.DAYS.between(currentShift.date.toJava(), nextShift.date.toJava()).toInt() - 1
            if (daysBetween >= 0) {
                ...
                val actualGap = Duration.between(currentEnd.toJava(), nextStart.toJava()).toHours()
