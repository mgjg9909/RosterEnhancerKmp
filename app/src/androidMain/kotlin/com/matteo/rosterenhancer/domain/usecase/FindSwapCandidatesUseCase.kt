...
class FindSwapCandidatesUseCase @Inject constructor(
    ...
    private fun validateRestRules(
        ...
        if (prevWork != null) {
            ...
            val gapMinutes = java.time.Duration.between(yEnd.toJava(), proposedStart.toJava()).toMinutes()
        ...
        if (nextWork != null) {
            ...
            val gapMinutes = java.time.Duration.between(proposedEnd.toJava(), tomStart.toJava()).toMinutes()
            val required = if (hasRestBetween) 35 else 11
            val requiredMinutes = required * 60L
            if (gapMinutes < requiredMinutes) {
