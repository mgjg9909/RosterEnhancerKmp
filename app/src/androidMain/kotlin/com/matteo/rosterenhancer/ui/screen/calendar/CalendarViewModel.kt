...
class CalendarViewModel constructor(
    ...
    init {
        if (com.matteo.rosterenhancer.BuildConfig.DEBUG) android.util.Log.d("CalendarViewModel", "Costruttore avviato")
    ...
    val myShiftsThisMonth: StateFlow<List<Shift>> = combine(
        ...
        if (com.matteo.rosterenhancer.BuildConfig.DEBUG) android.util.Log.d("CalendarViewModel", "FLOW_DEBUG: Mese=$month, Matricola='$matricola'")
        Pair(month, matricola) 
    }
        .flatMapLatest { (month, matricola) ->
            if (matricola.isBlank()) {
                if (com.matteo.rosterenhancer.BuildConfig.DEBUG) android.util.Log.w("CalendarViewModel", "DIAGNOSTICA: Matricola VUOTA, cerco turni per ID 'self' come fallback")
                ...
                repository.getShiftsForEmployeeInRange(
                    ...
                    if (com.matteo.rosterenhancer.BuildConfig.DEBUG) android.util.Log.d("CalendarViewModel", "DIAGNOSTICA: Mese=${month}, Matricola=${matricola}, TurniTrovati=${list.size}")
                }
            }
        }
    ...
    val shiftsForSelectedDay: StateFlow<List<Shift>> = combine(
        ...
        .flatMapLatest { (date, ui) ->
            if (date == null) flowOf(emptyList())
                ...
                if (com.matteo.rosterenhancer.BuildConfig.DEBUG) android.util.Log.d("CalendarViewModel", "DIAGNOSTICA: Giorno=${date}, Totali=${shifts.size}, Filtrati=${filtered.size}")
                filtered
            }
        }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
