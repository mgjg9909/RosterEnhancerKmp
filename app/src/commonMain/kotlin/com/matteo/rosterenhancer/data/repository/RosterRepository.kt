...
class RosterRepository(
    ...
    private val scraper: IRosterScraper,
    ...
    suspend fun setSelf(matricola: String) {
        ...
        println("RosterRepository: Identità Self impostata su matricola: $matricola (DB + DataStore)")
    }
    ...
    suspend fun syncWithWeb(): Result<String> = runCatching {
        ...
        val groupShifts = scraper.fetchGroupRoster().getOrElse { err ->
            println("RosterRepository: Turni gruppo non disponibili: ${err.message}")
            emptyList<Shift>()
        }
        ...
        if (!myId.isNullOrBlank() && myId != "self" && myId.all { it.isDigit() }) {
            println("RosterRepository: Auto-allineamento identità: impostazione matricola $myId come Self")
            setSelf(myId)
        }
    ...
    private suspend fun processRosterData(
        ...
        println("RosterRepository: Import completato: $inserted inseriti, $updated aggiornati, $skipped saltati (totale elaborati: ${shiftEntities.size})")
